...
def point_in_2Dbox(points_im, obj):
    ...
    point_filter = (points_im[:, 0] > obj[0]) & \
                   (points_im[:, 0] < obj[2]) & \
                   (points_im[:, 1] > obj[1]) & \
                   (points_im[:, 1] < obj[3])
    return point_filter
...
def calculate_ground_weakly(point_cloud, tran_lidar_to_cam, tran_cam_to_img, image_shape=None, thresh_ransac=0.15, back_cut=True, back_cut_z=-5.0):
    ''' Calculates the lidar point cloud, and optionally returns only the
        points that are projected to the image.
    ...
    if len(point_cloud) == 0:
        return np.array([[0],[0],[0]])
    if image_shape is not None:
        im_size = [image_shape[0], image_shape[0]]
    else:
        im_size = [1242, 375]

    # Only keep points in front of camera (positive z)
    if back_cut:
        point_cloud = point_cloud[point_cloud[:,2] > back_cut_z]   # N x camera frame 3
    
    planeDiffThreshold = thresh_ransac
    temp = torch.sort(point_cloud[:,1])[0][int(point_cloud.shape[0]*0.75)].item()
    cloud = point_cloud[point_cloud[:,1]>temp]
    points_torch = point_cloud
    mask_all = torch.ones(points_torch.shape[0]).to(points_torch.device)
    final_sample_points = None
    ...
    for i in range(5):
        ...
        for iteration in range(min(cloud.shape[0], 100)):
            ...
            plane = torch.from_numpy(fitPlane(sampledPoints)).float().to(sampledPoints.device)
            div = np.linalg.norm(plane.cpu().detach().numpy())
            diff = torch.abs(torch.matmul(points_torch, plane) - torch.ones(points_torch.shape[0]).to(points_torch.device)) / div #Why?
            inlierMask = diff < planeDiffThreshold # True if ground point?
            numInliers = inlierMask.sum()
            if numInliers > best_len and torch.abs(torch.dot(plane/div, torch.tensor([0,1,0]).float().to(plane.device))) > 0.9: #Why?
                mask_ground = inlierMask
                best_len = numInliers
                best_plane = plane
                final_sample_points = sampledPoints
        
        mask_all *= 1 - mask_ground.int()  # True if not ground?
    return mask_all, final_sample_points
...
def get_point_cloud_weakly_version(points, tran_lidar_to_cam, tran_cam_to_img, image_shape=None, objects=None, back_cut=True, back_cut_z=-5.0):
    ...
    # Only keep points in front of camera (positive z)
    if back_cut: 
        point_cloud = point_cloud[point_cloud[:,2] > back_cut_z].T   # camera frame  x N
    else:
        point_cloud = point_cloud.T # 4 x N

    points2d = torch.matmul(point_cloud.T, tran_cam_to_img.T)

    depth = points2d[..., 2:3]
    point_in_im = points2d[..., :2] / depth  
    ...
    return point_cloud[:3].T, object_filter
...
def region_grow_weakly_version(pc, mask_search, mask_origin, thresh, ratio=0.8):
    
    pc_search = pc[mask_search==1]
    mask = mask_origin.detach().clone()
    best_len = 0
    mask_best = torch.zeros((pc.shape[0])).float().to(pc.device)
    while mask.sum() > 0:
        seed = pc[mask==1][0]
        seed_mask = torch.zeros((pc_search.shape[0])).float().to(pc.device)
        seed_mask_all = torch.zeros((pc.shape[0])).float().to(pc.device)
        seed_list = [seed]
        flag = 1
        while len(seed_list) > 0:
            temp = seed_list.pop(0)
            dis = np.linalg.norm(pc_search.detach().cpu().numpy() - temp.detach().cpu().numpy(), axis=-1)
            index = torch.argmin(torch.from_numpy(dis))
            seed_mask[index] = 1
            
            valid_mask = torch.from_numpy(dis < thresh).to(pc.device) * (1 - seed_mask)
            seed_list += [val_seed for val_seed in (pc_search[valid_mask==1])]
            seed_mask[valid_mask == 1] = 1
            seed_mask_all[mask_search==1] = seed_mask
            if ratio is not None and (seed_mask_all*mask_origin).sum().item() / seed_mask.sum().item() <ratio:
                flag = 0
                break
